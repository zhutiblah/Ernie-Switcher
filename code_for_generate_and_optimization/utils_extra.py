...
def Dimer_split_seqs(seq):
    """
    Returns [[monomer indices(seq_len)], [dimer indices(seq_len)]]
    Consistent with original version: illegal characters filled with -1
    """
    t = text_build_vocab()
...
def write_good_record(dict1, dict2, file_path):
    ...
    print(f"Record successfully written to: {file_path}")
...
def evaluate_regression_metrics(output, label):
    ...
    if np.isnan(output).any() or np.isnan(label).any():
        print("Warning: NaN values exist in input data, may affect evaluation results.")
...
def find_and_set_gpu_for_paddle(target_gpus=[1, 2], min_free_mem_gb=50, max_gpu_util=5):
    """
    GPU selection tool called by training scripts. Maintains consistency with original logic.
    """
    if paddle is None:
    ...
    while selected_gpu is None and retries < max_retries:
        ...
        if available_gpus_for_task:
            ...
            if retries < max_retries:
                ...
                print(f"Maximum retries reached ({max_retries}), no available GPU found.")
    return None
...
def setup_rna_ernie_model(vocab_path: str,
                          ...
                          st_pos: int = 0):
    """
    For use in prediction phase (and when immediate processing is needed):
    - Load ErnieModel
    - Construct a BatchConverter (equivalent to the converter used during training)
    Returns: (ernie_model, batch_converter)
    """
...
def tokenize_sequence(sequence: str, nuc_tokenizer: NUCTokenizer, max_seq_len: int = 115) -> list:
    """
    Following BatchConverter logic:
    - Truncate to max_seq_len - 2
    - upper + U->T
    - seq2input_ids
    Returns only unpadded input_ids; padding is handled by upper layer or BatchConverter
    """
...
def extract_rna_ernie_embeddings(mRNAs_list,
    ...
    - Uses BatchConverter
    - Uses ErnieModel.from_pretrained
    - Saves "last layer hidden state of entire sequence" (shape: [seq_len, hidden])
    - No [CLS] pooling, ensuring no changes needed to original training code
    """
    if ErnieModel is None or paddle is None:
        raise ImportError("PaddlePaddle/PaddleNLP not installed, cannot extract RNA-Ernie embeddings.")

    print("\n--- Starting RNA-Ernie embedding extraction (batch saving) ---")
    if not paddle.is_compiled_with_cuda():
        print("Warning: Not in CUDA environment, may be slow.")
    ...
    print(f"RNAErnie model hidden size: {ernie_hidden_size}")
    print(f"RNAErnie max sequence length: {max_seq_len}")
    ...
    with paddle.no_grad():
        for batch_idx, (batch_names, _, inputs_ids) in enumerate(converter(data_for_ernie)):
            ...
            if (batch_idx + 1) % 10 == 0:
                print(f"Processed batches {batch_idx + 1}, temporarily accumulated {len(current_batch_data_temp)} entries.")
            ...
            if len(current_batch_data_temp) >= save_batch_threshold:
                ...
                print(f"Saved batch file: {file_prefix}, containing {len(current_batch_data_temp)} entries.")
                current_batch_data_temp = []
    ...
    if current_batch_data_temp:
        ...
        saved_embedding_files.append(file_prefix)
        print(f"Saved last batch file: {file_prefix}, containing {len(current_batch_data_temp)} entries.")
...
def load_rna_ernie_embeddings(saved_embedding_files, valid_original_indices, save_dir="ernie_embeddings_batches"):
    """
    Load and return sorted by valid_original_indices
    Returns final_embeddings: [N, seq_len, hidden]
    """
    ...
    if not all_loaded_embeddings_list:
        print("Error: No embedding content loaded successfully!")
        return None
...
def extract_rna_ernie_single(sequence: str,
    ...
    """
    Extract embedding for a single sequence using BatchConverter logic.
    Returns "entire sequence last layer hidden state" (shape: [seq_len, hidden]),
    consistent with training extraction.
    """
    if paddle is None:
...
def extract_rna_ernie_for_prediction(mRNAs_list,
                                     ...
                                     max_seq_len=115):
    """
    RNA-Ernie embedding extraction function designed for prediction
    
    Args:
        mRNAs_list: List of mRNA sequences
        vocab_path: Vocabulary path
        ernie_model_path: Ernie model path
        batch_size: Batch size
        k_mer: k-mer parameter
        max_seq_len: Maximum sequence length
    
    Returns:
        embeddings: numpy array, shape [N, seq_len, hidden_size]
    """
    if ErnieModel is None or paddle is None:
        raise ImportError("PaddlePaddle/PaddleNLP not installed, cannot extract RNA-Ernie embeddings.")
