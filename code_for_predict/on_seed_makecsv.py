...
def train(params, features_array, ernie_embeddings, structure_array, labels_array, random_seed=42, model_init_seed=None):
    ...
    csv_filename = os.path.join(
        loss_save_dir,
        f'on_all_folds_loss_record_{loss_kind}_{embedding_type}_seed{random_seed}{model_init_str}.csv'
    )
    ...
    result_file = f'../result/on_good_record_metric_{loss_kind}_ori_dim_structure.txt'
    if ernie_embeddings is not None:
        result_file = f'../result/on_good_record_metric_{loss_kind}_ernie_structure.txt'
    
    print(f"Current K-Fold results will be written to: {result_file}")
    print(f"Each epoch record will be appended in real-time to: {csv_filename}")
...
if __name__ == '__main__':
    ...
    print("\n" + "="*80)
    print(f"🎲 Using global random seed = {GLOBAL_SEED} for all data splitting and training (only changing model init seed)".center(80))
    print("="*80)
    ...
    for init_seed in MODEL_INIT_SEEDS:
        ...
        print(f"\n✓ Global seed {GLOBAL_SEED} + init seed {init_seed} completed, best PCC: {best_pcc:.6f}")
    
    print("\n" + "="*80)
    print("📊 Summary of all random seed experiments".center(80))
    print("="*80)
    ...
    print(f"Maximum: {results_df['best_pcc'].max():.6f}")
    print(f"Minimum: {results_df['best_pcc'].min():.6f}")
    
    summary_file = '../result/multi_seed_summary.csv'
    ...
    all_loss_dfs = []
    pattern = os.path.join(loss_save_dir, f'on_all_folds_loss_record_{loss_kind}_{embedding_type}_seed*.csv')
    ...
    if all_loss_dfs:
        ...
        combined_file = os.path.join(
            loss_save_dir,
            f'on_all_seeds_combined_{loss_kind}_{embedding_type}.csv'
        )
        combined_df.to_csv(combined_file, index=False)
        print(f"✓ All seed loss records merged and saved to: {combined_file}")
