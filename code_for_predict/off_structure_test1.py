...
def train(params, features_array, ernie_embeddings, structure_array, labels_array):
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(features_array)):
        ...
        for epoch in range(params['train_epochs_num']):
            ...
            if epoch % 10 == 0:
                print(f"Epoch[{epoch}/{params['train_epochs_num']}] Train loss: {loss_train_one_epoch/len(train_loader):.6f} MSE: {loss_mse/len(train_loader):.6f} Pearson: {loss_pier/len(train_loader):.6f}")
            ...
            if epoch % 10 == 0:
                print(f"Epoch[{epoch}/{params['train_epochs_num']}] Test loss: {loss_test_one_epoch/len(test_loader):.6f} PCC: {correlation_coefficient:.6f}")
