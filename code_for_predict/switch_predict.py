...
import numpy as np
import pandas as pd
from utils import Dimer_split_seqs, extract_rna_ernie_for_prediction, extract_rna_ernie_single, setup_rna_ernie_model
import paddle
import subprocess
import re
...
def find_most_available_gpu():
    try:
        ...
        print(f"Detected most available GPU: {most_available_gpu_index} (Used memory: {gpus_info[0]['used']} MiB / Total memory: {gpus_info[0]['total']} MiB)")
        ...
        print(f"Error: 'nvidia-smi' command execution failed. Error message: {e}")
        return -1
...
class ToeholdPredictorSimplified:
    ...
    def __init__(self, 
        ...
        print(f"\nLoading PyTorch prediction model to {self.device}...")
    ...
    def extract_features_for_batch(self, mRNAs: list):
        ...
        for i, mRNA in enumerate(mRNAs):
            if len(mRNA) != 115:
                print(f'⚠️  Sequence {i} has length {len(mRNA)}, skipping')
                continue
        ...
        ernies = None
        if self.use_rna_ernie:
            print("\nBatch extracting RNA-Ernie embeddings...")
    ...
    def predict_from_csv(self, csv_path, output_path=None, batch_size=32):
        ...
        for idx, row in df.iterrows():
            try:
                ...
                print(f"❌ Failed to construct sequence for row {idx}: {e}")
