...
def make_dataset_sequences_bio(mRNAs, ons, offs, on_offs, out_label='on'):
...
def train(params, features_array, labels_array):
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(features_array)):
        ...
        gen = predict_transformerv2.Predict_translation_off(params=params).to(device)

        initialize_weights(gen)
        print('successful compose simple prediction model')
        ...
        for epoch in range(params['train_epochs_num']):
            ...
            if correlation_coefficient > pcc:
                ...
                if loss_kind == 'pearson':
                    torch.save(gen,'../model/{0}_pearson_predict_{1}_metric={2:.6f}.pth'.format(prediction_flag, epoch, correlation_coefficient))
                
                elif loss_kind == 'pearson_mse':
                    torch.save(gen,'../model/{0}_pearson_mse_predict_{1}_metric={2:.6f}.pth'.format(prediction_flag, epoch, correlation_coefficient))
                
                elif loss_kind == 'mse':
                    torch.save(gen,'../model/{0}_mse_predict_{1}_metric={2:.6f}.pth'.format(prediction_flag, epoch, correlation_coefficient))
        ...
        if loss_kind == 'pearson':
            write_good_record(dict1=params,dict2=dict2,file_path=f'../result/{prediction_flag}_good_record_metric_pearson_ori_dim.txt')
        
        elif loss_kind == 'pearson_mse':
            write_good_record(dict1=params,dict2=dict2,file_path=f'../result/{prediction_flag}_good_record_metric_pearson_mse_ori_dim.txt')
        
        elif loss_kind == 'mse':
            write_good_record(dict1=params,dict2=dict2,file_path=f'../result/{prediction_flag}_good_record_metric_mse_ori_dim.txt')
