...
def train(params, features_array, ernie_embeddings, structure_array, labels_array):
    ...
    csv_filename = os.path.join(
        loss_save_dir,
        f'adjust1_off_all_folds_loss_record_{loss_kind}_{embedding_type}.csv'
    )
        ...
        for epoch in range(params['train_epochs_num']):
            ...
            for batch_data in train_loader:
                ...
                if loss_kind == 'pearson':
                    ...
                    raise ValueError('Loss function type error')
            ...
            if epoch % 10 == 0:
                print(f"Epoch[{epoch}/{params['train_epochs_num']}] Train loss: {avg_train_loss:.6f} MSE: {avg_train_mse:.6f} Pearson: {avg_train_pearson:.6f}")
            ...
            if epoch % 10 == 0:
                print(f"Epoch[{epoch}/{params['train_epochs_num']}] Test loss: {avg_test_loss:.6f} PCC: {correlation_coefficient:.6f}")
            ...
            if correlation_coefficient > pcc:
                ...
                model_name = f'off_{loss_kind}_structure_{epoch}_pcc={correlation_coefficient:.4f}.pth'
                
                if ernie_embeddings is not None:
                    model_name = f'off_{loss_kind}_ernie_structure_{epoch}_pcc={correlation_coefficient:.4f}.pth'
                    
                torch.save(gen, f'../model/{model_name}')
    ...
    print(f"\n{'='*60}")
    print(f"Loss records for all folds have been appended in real-time to unified file:")
...
if __name__ == '__main__':
    ...
    print('device =', device)
    print(f"\n========== Parameter Details ==========")
    print(f"Using RNA-Ernie: {USE_RNA_ERNIE}")
    print(f"embedding_dim1: {params['embedding_dim1']}")
    print(f"K-Fold count: {k_folds}")
    print(f"Loss function type: pearson_mse")
    print(f"================================\n")

    loss_kind = 'pearson_mse'
    print(f"ernie_array shape passed to train: {ernie_array.shape if ernie_array is not None else 'None'}")
    
    print("\n" + "="*60)
    print("Starting training (fixed parameter combination)")
    print("="*60)
    train(params, features_array=features_array, ernie_embeddings=ernie_array, 
          structure_array=structure_array, labels_array=labels_array)
